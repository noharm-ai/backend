...
def get_antimicrobial_levels(sctids: list) -> dict:
    ...
    Read when the culture card states how aggressive each antimicrobial of the
    antibiogram is (services/culture_service): a substance that is missing from
    the result, or one that was never curated, has no classification.
    """
    ...
    rows = (
        db.session.query(Substance.id, Substance.atb_level)
        .filter(Substance.id.in_(ids))
        .all()
    )

    return {int(row.id): row.atb_level for row in rows}
