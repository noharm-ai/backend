...
def test_get_drugs_by_idSegmento(client):
    ...
    segDrugs = session.query(Outlier.idDrug.label('idDrug'))\
...
def test_get_drugs_by_idSegment_and_qParam(client):
    ...
    drugsNum = session.query(Drug).filter(Drug.name.ilike("%a%")).filter(Drug.id.in_(segDrugs)).count()
...
def test_get_outliers_by_segment_and_drug(client):
    ...
    outlierNum = session.query(Outlier).filter_by(idSegment = testIdSegment, idDrug = testIdDrug).count()
