...
class TestOnlyLatestExpireDate:
    """config.onlyLatestExpireDate — restrict a protocol to the newest date group

    An aggregated prescription is evaluated once per expire-date group. Some
    protocols only make sense against what is currently prescribed, so the
    config can ask to be tested against the most recent group alone. The flag
    is per protocol: two protocols in the same prescription can disagree, and a
    config without the key keeps being tested against every group.
    """

    def test_latest_expire_date_picks_the_newest_group(self):
        """Group keys are ISO dates, so the newest one is the max"""
        assert (
            alert_protocol_service.get_latest_expire_date(
    ...
    def test_latest_expire_date_of_an_empty_split_is_none(self):
        """A prescription with no drugs has no group to elect"""
        assert (
            alert_protocol_service.get_latest_expire_date(drugs_by_expire_date={})
            is None
        )
    ...
    def test_flag_reading_defaults_to_false(self, config, expected):
        ...
        assert alert_protocol_service.only_latest_expire_date(config=config) is expected

    def test_flagged_protocol_runs_only_on_the_newest_group(self, fake_engine):
        """The older date group is skipped, but its bucket is still returned"""
        fake_engine.alerts_by_protocol = {5: {"message": "fired"}}
    ...
    def test_group_order_does_not_decide_the_newest_group(self, fake_engine):
        """The newest date wins even when it is not the last group built"""
        fake_engine.alerts_by_protocol = {5: {"message": "fired"}}
        ...
        result, _ = _run(
            ...
            drug_list=[
                ...
                _drug(2, expire_date=datetime(2024, 3, 11, 10, 0)),
            ],
            prescription=_prescription(agg=True),
        )
    ...
    def test_flag_is_read_per_protocol(self, fake_engine):
        ...
        fake_engine.alerts_by_protocol = {
            5: {"message": "latest only"},
            6: {"message": "every group"},
        }
        ...
        assert result["2024-03-11"] == [{"message": "every group", "id": 6}]
        assert result["2024-03-12"] == [
            {"message": "latest only", "id": 5},
            {"message": "every group", "id": 6},
        ]

    def test_config_without_the_key_still_runs_on_every_group(self, fake_engine):
        """Back compatibility: protocols stored before the field are untouched"""
        fake_engine.alerts_by_protocol = {5: {"message": "fired"}}
    ...
    def test_flagged_item_protocol_reports_a_single_item_alert(self, fake_engine):
        """An item protocol fires once instead of once per date group"""
        fake_engine.alerts_by_protocol = {7: {"message": "item"}}
        ...
        assert result["items"] == [{"message": "item", "id": 7}]

    def test_single_group_prescription_is_unaffected(self, fake_engine):
        """An individual prescription has one group, which is also the latest"""
        fake_engine.alerts_by_protocol = {5: {"message": "fired"}}
