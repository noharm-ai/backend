...
def test_upsert_persists_only_latest_expire_date(
    ...
    It is the field that decides how many expire date groups the protocol is
    tested against, so the editor and the prescription view must both see the
...
def test_upsert_defaults_only_latest_expire_date_to_false(
    ...
    """A config saved without the field keeps the every-date-group behavior."""
    response = client.post(
        UPSERT_URL,
