    ...
    def getDrugType(self, pDrugs, source, checked=False, suspended=False):
        for pd in self.drugList:
            ...
            pDrugs.append({
                ...
                'dose': pd[0].dose,
                'measureUnit': { 'value': pd[2].id, 'label': pd[2].description } if pd[2] else { 'value': pd[0].idMeasureUnit, 'label': pd[0].idMeasureUnit },
                'frequency': { 'value': pd[3].id, 'label': pd[3].description } if pd[3] else { 'value': pd[0].idFrequency, 'label': pd[0].idFrequency },
