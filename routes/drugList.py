...
class DrugList():
    ...
    def sumAlerts(self):
        self.alertStats['exams'] = self.alertStats['exams'] + self.alertStats['kidney'] + self.alertStats['liver'] + self.alertStats['platelets']
    ...
    def getDrugType(self, pDrugs, source, checked=False, suspended=False):
        for pd in self.drugList:
            ...
            if (pd[2] != None and pd[6] != None and pd[6].division != None):
                measureUnitFactor = 1
                measureUnitConvert = MeasureUnitConvert.query.get((pd[2].id, pd[0].idDrug, pd[0].idSegment)) #TODO: multiple lazy loading
            ...
            if not bool(pd[0].suspendedDate):
                if self.exams and pd[6]:
                    ...
                    if pd[6].useWeight and pd[0].dose:
                        ...
                        doseWeightStr = strFormatBR(round(pd[0].dose / float(weight),2)) + ' ' + pdUnit + '/Kg'
            ...
            pDrugs.append({
                ...
                'dose': pd[0].dose,
                'measureUnit': { 'value': pd[2].id, 'label': pd[2].description } if pd[2] else { 'value': strNone(pd[0].idMeasureUnit), 'label': strNone(pd[0].idMeasureUnit) },
                'frequency': { 'value': pd[3].id, 'label': pd[3].description } if pd[3] else { 'value': strNone(pd[0].idFrequency), 'label': strNone(pd[0].idFrequency) },
