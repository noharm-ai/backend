...
def getSegmentsId(idSegment, idHospital = None):
    ...
    q_department = db.session.query(func.count(sd.id).label('count'))\
                      .select_from(sd)\
                      .filter(sd.idHospital == idHospital)\
                      .filter(sd.idDepartment == Department.id)

    if (idHospital != None):
        departments = db.session.query(Department, SegmentDepartment, q_department.as_scalar().label('total'))\
                    .outerjoin(\
                        SegmentDepartment,\
                        and_(\
        ...
        departments = db.session.query(Department, SegmentDepartment, literal_column('0').label('total'))\
                    .join(\
                        SegmentDepartment,\
                        and_(\
                            SegmentDepartment.idDepartment == Department.id,\
